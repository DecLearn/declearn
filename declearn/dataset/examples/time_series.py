...
def load_raw_dataframes(params : EMGDatasetConfigs)-> Iterator[pd.DataFrame]:
    ...
    Args:
        source (Union[str, bytes]): _description_
        subject_indx (int): _description_
        action (str): _description_

    Returns:
        _type_: _description_
    """
    #TODO: read directly from the zip file and yield dataframes equivalent to the subjects
    source = os.path.join(params.folder, f'{params.zip_name}.zip')
    with zipfile.ZipFile(source) as zfile:
        #TODO: extract files that are included in the list of subjects with the defined action 
